...
* **只有从 Swap Chain 上获取到一帧之后我们才可以开始进行绘制（1->2）；**
...
* `timeout`：超时时间，单位为ns。
* `semaphore` 和 `fence`。指定同步对象。这里由于先不考虑 CPU 端同步，因此只使用之前声明的`imageAvailableSemaphore` 对象，表示**只有从 Swap Chain 上获取到一帧之后我们才可以开始进行绘制**。
...
>  至于到底获取的是哪一个图像，这就得看之前 *交换链 (Swap Chain)*  一章中提到的 Swap Chain 配置方法了。
...
* Swap Chain 的图像会被 CPU 拿到一张不剩。此时 `vkAcquireNextImageKHR` 将会阻塞，直到超时（通过 timeout 指定，超时后返回 `VK_TIMEOUT`）。这还算好的——但我们依然不能依赖这种方法来做同步；
* 如果我们用的是一套资源，例如之后介绍的 Descriptor，**在为了新的一帧更新这套资源时，这套资源也正在被 GPU 所渲染当前帧，造成冲突，从而产生未定义行为。**这很危险。

因此我们需要使用上述提到的 Fence 机制，来让（可能会快一些的）CPU 等待 GPU 干完活在开始下达新的渲染任务。

#### 使用 Fence + vkAcquireNextImageKHR 进行阻塞
...
当然，由于我们并没有考虑呈现步骤，因此在新一帧的 `vkAcquireNextImageKHR` 时可能依然会被阻塞，此时这也可以被看作是一种 CPU - GPU 在呈现上的同步了。

使用方法如下：

1. 使用 `vkCreateFence` 创建 VkFence 对象。
2. 将创建好的对象传入 `vkQueueSubmit` 的对应参数中。
3. 在 CPU 进行下一轮绘制的一开始，使用 `vkWaitForFences` 进行阻塞等待。
