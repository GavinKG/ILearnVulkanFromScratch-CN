...
* 使用 `VkPipelineLayout` 配置**流水线布局**。这里需要首先填写`VkPipelineLayoutCreateInfo `来创建一个 `VkPipelineLayout`（之后也需要销毁），创建的过程中需要资源描述集布局 Descriptor Set Layout，之后的**资源描述**一章中会提到；
...
对于驱动来说，创建流水线是一个很复杂的过程，例如编译 Shader 到各家显卡原生支持的机器码。因此除非特殊情况，流水线的创建尽量要在 Draw Loop 运行之前，或者 Loading 的时候创建。此时可能需要开发者提前收集到程序中会用到的所有流水线。
