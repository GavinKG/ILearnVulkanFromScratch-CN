...
其实整个流水线的配置和绘制物体所使用的 Shader 和摄像机配置（视口、MSAA 等）两个概念有着极大的关系，在一个摄像机配置下渲染同一个材质的物体，流水线可以不用修改。
...
对于游戏引擎来说，将需要绘制的内容按照 pipeline 打包成一起绘制可以减少因为切换流水线（`vkCmdBindPipeline`）导致的潜在的性能损失。
